import org.specs2.mutable._
import org.specs2.runner._
import org.junit.runner._
import play.api.test._
import play.api.test.Helpers._

@RunWith(classOf[JUnitRunner])
class ApplicationSpec extends Specification {

  "Application" should {

    "send 404 on a bad request" in new WithApplication {
      route(app, FakeRequest(GET, "/search")) must beNone
    }

    "get greeting" in new WithApplication {
      val greeting = route(app, FakeRequest(GET, "/greeting")).get
      status(greeting) must equalTo(OK)
      contentAsString(greeting) must equalTo("Hello")
    }

    "get greeting must include request parameter" in new WithApplication {
      val greeting = route(app, FakeRequest(GET, "/greeting2?param=World")).get
      status(greeting) must equalTo(OK)
      contentAsString(greeting) must equalTo("Hello World")
    }

    "render the index page" in new WithApplication {
      val home = route(app, FakeRequest(GET, "/")).get
      status(home) must equalTo(OK)
      contentType(home) must beSome.which(_ == "text/html")
      contentAsString(home) must contain ("Your new application is ready.")
    }
  }
}
